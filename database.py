...
def submit(amount, month_clicked):

    conn = sqlite3.connect('name1.db')
    c = conn.cursor()

    c.execute("INSERT INTO expenditures VALUES(:amount, :month)",{'amount': amount.get(),'month': month_clicked.get()})
...
def gui_db_button(gui_db, amount, month_clicked):

    button_submit = Button(gui_db, text="Submit", bg="gray", height=1, width=15, command=lambda:submit(amount, month_clicked))
...
def database_main():
    ...
    amount.grid(row=5, column=0, padx=20, pady=5)
    #gui_db_Entry(gui_db)
    gui_db_button(gui_db, amount ,month_clicked)
    gui_db_Label(gui_db)
    show_records()
    gui_db.mainloop()
